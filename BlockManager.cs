...
public partial class BlockManager : Node
{
	...
	public Vector2I BlockTextureSize { get; } = new(64, 64);
	...
	public override void _Ready()
	{
		...
		var blockTextures = new[]{Air, Stone, Dirt, Grass}.SelectMany(block => block.Textures).Where(texture => texture != null).Distinct().ToArray();

		for (var i = 0; i < blockTextures.Length; i++)
		{
			var texture = blockTextures[i];
			_atlasLookup.Add(texture, new Vector2I(i % _gridWidth, Mathf.FloorToInt((float) i / _gridWidth)));
		}

		_gridHeight = Mathf.CeilToInt((float) blockTextures.Length / _gridWidth);
